import { INCOTERMS } from './incoterms'

const GROUP_TERMS = {
  E: INCOTERMS.filter(t => t.group === 'E').map(t => t.code),
  F: INCOTERMS.filter(t => t.group === 'F').map(t => t.code),
  C: INCOTERMS.filter(t => t.group === 'C').map(t => t.code),
  D: INCOTERMS.filter(t => t.group === 'D').map(t => t.code),
}

function groupAccuracy(state, codes) {
  let totalCorrect = 0, totalAttempted = 0
  for (const code of codes) {
    const m = state.incotermsMastered[code]
    if (m) { totalCorrect += m.correct; totalAttempted += m.total }
  }
  return totalAttempted > 0 ? Math.round((totalCorrect / totalAttempted) * 100) : 0
}

export const BADGES = [
  // Mastery (4)
  {
    id: 'group-e-master', name: { id: 'Master Grup E', en: 'E-Group Master' },
    description: { id: 'Kuasai EXW (Departure)', en: 'Master EXW (Departure)' },
    icon: 'Factory',
    thresholds: { bronze: 50, silver: 75, gold: 90 },
    check: (state) => groupAccuracy(state, GROUP_TERMS.E),
  },
  {
    id: 'group-f-master', name: { id: 'Master Grup F', en: 'F-Group Master' },
    description: { id: 'Kuasai FCA, FAS, FOB', en: 'Master FCA, FAS, FOB' },
    icon: 'Ship',
    thresholds: { bronze: 50, silver: 75, gold: 90 },
    check: (state) => groupAccuracy(state, GROUP_TERMS.F),
  },
  {
    id: 'group-c-master', name: { id: 'Master Grup C', en: 'C-Group Master' },
    description: { id: 'Kuasai CFR, CIF, CPT, CIP', en: 'Master CFR, CIF, CPT, CIP' },
    icon: 'Globe',
    thresholds: { bronze: 50, silver: 75, gold: 90 },
    check: (state) => groupAccuracy(state, GROUP_TERMS.C),
  },
  {
    id: 'group-d-master', name: { id: 'Master Grup D', en: 'D-Group Master' },
    description: { id: 'Kuasai DAP, DPU, DDP', en: 'Master DAP, DPU, DDP' },
    icon: 'MapPin',
    thresholds: { bronze: 50, silver: 75, gold: 90 },
    check: (state) => groupAccuracy(state, GROUP_TERMS.D),
  },
  // Performance (4)
  {
    id: 'streak-hunter', name: { id: 'Pemburu Streak', en: 'Streak Hunter' },
    description: { id: 'Raih jawaban benar beruntun', en: 'Achieve consecutive correct answers' },
    icon: 'Flame',
    thresholds: { bronze: 5, silver: 10, gold: 20 },
    check: (state) => state.bestStreak,
  },
  {
    id: 'sharpshooter', name: { id: 'Penembak Jitu', en: 'Sharpshooter' },
    description: { id: 'Jaga akurasi total tetap tinggi', en: 'Maintain high overall accuracy' },
    icon: 'Target',
    thresholds: { bronze: 70, silver: 85, gold: 95 },
    check: (state) => state.totalAttempted > 0
      ? Math.round((state.totalCorrect / state.totalAttempted) * 100) : 0,
  },
  {
    id: 'speed-demon', name: { id: 'Setan Kecepatan', en: 'Speed Demon' },
    description: { id: 'Selesaikan banyak skenario', en: 'Complete many scenarios' },
    icon: 'Zap',
    thresholds: { bronze: 10, silver: 50, gold: 100 },
    check: (state) => state.scenariosCompleted.length,
  },
  {
    id: 'point-collector', name: { id: 'Kolektor Poin', en: 'Point Collector' },
    description: { id: 'Kumpulkan poin XP', en: 'Accumulate XP points' },
    icon: 'Coins',
    thresholds: { bronze: 500, silver: 2000, gold: 5000 },
    check: (state) => state.score,
  },
  // Special (2)
  {
    id: 'perfect-run', name: { id: 'Lari Sempurna', en: 'Perfect Run' },
    description: { id: 'Selesaikan skenario tanpa salah', en: 'Complete scenarios with zero mistakes' },
    icon: 'CheckCircle',
    thresholds: { bronze: 3, silver: 5, gold: 10 },
    check: (state) => state.perfectScenarios || 0,
  },
  {
    id: 'globe-trotter', name: { id: 'Penjelajah Global', en: 'Globe Trotter' },
    description: { id: 'Coba banyak skenario unik', en: 'Try unique scenarios' },
    icon: 'Globe2',
    thresholds: { bronze: 5, silver: 15, gold: 26 },
    check: (state) => state.scenariosCompleted.length,
  },
]

export function getBadgeTier(badge, state) {
  const value = badge.check(state)
  if (value >= badge.thresholds.gold) return 'gold'
  if (value >= badge.thresholds.silver) return 'silver'
  if (value >= badge.thresholds.bronze) return 'bronze'
  return null
}

export function computeAllBadges(state) {
  return BADGES.map(badge => ({
    ...badge,
    tier: getBadgeTier(badge, state),
    value: badge.check(state),
  }))
}
